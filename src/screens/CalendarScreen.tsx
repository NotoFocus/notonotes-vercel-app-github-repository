...
export default function CalendarScreen() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 font-sans text-slate-200">
      ...
      <div className="px-6 py-6 flex-1 overflow-y-auto no-scrollbar pb-6">
        ...
        <div className="flex justify-between items-center mb-6 px-1">
          ...
          <h2 className="text-xl font-bold text-slate-50 tracking-tight">{monthNames[currentMonth]} {currentYear}</h2>
        ...
        <div className="grid grid-cols-7 mb-4">
          {daysOfWeek.map((day, i) => (
            <div key={i} className="text-center text-[10px] uppercase font-bold text-slate-500 tracking-widest">
              {day}
            </div>
          ))}
        </div>
        ...
        {/* Streak Info */}
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
             <div className="w-12 h-12 bg-orange-500/10 text-orange-400 rounded-2xl flex items-center justify-center">
               <Flame className="w-6 h-6" />
             </div>
             <div>
               <h4 className="text-slate-50 font-bold">{t('streak')}</h4>
               <p className="text-xs text-slate-400">{t('streakKeep')}</p>
             </div>
          </div>
          <div className="flex flex-col items-end">
             <span className="text-2xl font-black text-orange-400 tracking-tighter">{streak}</span>
             <span className="text-[10px] uppercase font-bold text-orange-400/70 tracking-widest">{t('days')}</span>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex bg-slate-900 border border-slate-800 rounded-xl p-1 mb-6">
            {['Harian', 'Mingguan', 'Bulanan', 'Tahunan'].map((type, idx) => {
              ...
              <button 
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${viewType === type ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50'}`}
              >
                {displayType[idx]}
              </button>
            )})}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 mb-6">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h4 className="text-slate-50 font-bold">{t('taskSummary') || 'Ringkasan Tugas'}</h4>
                <p className="text-xs text-slate-400">{t('todayAchievement') || 'Pencapaian hari ini'}</p>
              </div>
            </div>

            <div className="flex gap-4">
              <div className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl p-4 text-center">
                <div className="text-2xl font-bold text-slate-50 mb-1">{selectedTasks.length}</div>
                <div className="text-[10px] uppercase tracking-widest font-bold text-slate-500">{t('total') || 'Total'}</div>
              </div>
              <div className="flex-1 bg-slate-950 border border-emerald-900/50 rounded-2xl p-4 text-center">
                ...
                <div className="text-[10px] uppercase tracking-widest font-bold text-emerald-600">{t('completed') || 'Selesai'}</div>
              </div>
              <div className="flex-1 bg-slate-950 border border-orange-900/50 rounded-2xl p-4 text-center">
                <div className="text-2xl font-bold text-orange-400 mb-1">{activeCount}</div>
                <div className="text-[10px] uppercase tracking-widest font-bold text-orange-600">{t('active') || 'Aktif'}</div>
              </div>
            </div>

            {selectedTasks.length > 0 && (
              <div className="mt-6 pt-6 border-t border-slate-800">
                <h5 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 text-center">{t('completionPercentage') || 'Persentase Selesai'}</h5>
...
const Day: React.FC<{ num: string, mute?: boolean, active?: boolean, isToday?: boolean, onClick?: () => void }> = ({ num, mute = false, active = false, isToday = false, onClick }) => {
  return (
    <div className="flex items-center justify-center">
      <div onClick={onClick} className={`w-8 h-8 flex items-center justify-center rounded-xl text-xs font-bold transition-all relative ${
        ...
        {num}
        {isToday && !active && <div className="absolute -bottom-1 w-1 h-1 bg-indigo-500 rounded-full"></div>}
      </div>
    </div>
  );
}
