...
if "function CustomKeypad" not in content:
    content = content.replace("function App() {", keypad_component + "\nfunction App() {")

pattern_backup = r'<div className="relative w-full max-w-\[200px\] mx-auto mb-6">.*?<input[^>]*value={enteredAutoBackupPin}[^>]*>.*?</div>\s*</div>'
replacement_backup = """<CustomKeypad 
              ...
              />"""
content = re.sub(pattern_backup, replacement_backup, content, flags=re.DOTALL)
